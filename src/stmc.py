...
def read_submotions(path, fps=20.0, start_at_zero=True):
...
def process_timeline(timeline, interval_overlap, extend=True, uncond=True, bodyparts=True):
    ...
    if not bodyparts:
        ...
        n_uncond = 0 # if uncond else len(timeline)
        texts = [x.text for x in timeline] + [""] * n_uncond
        info_ = {
            # timeline
            "timeline": timeline,
            "all_intervals": timeline,
            # batch
            "n_uncond": 0,
            "texts": texts,
            "max_t": max_t,
...
def process_submotions(timelines, interval_overlap, extend=True, uncond=True, bodyparts=True): 
    infos = defaultdict(list)

    count = 0 # MYCODE
    for timeline in timelines:
        info = process_timeline(
            timeline, interval_overlap, extend=extend, uncond=uncond, bodyparts=bodyparts
        )
        count += 1
        # Merge the infos
...
def bada_bim(output_core, output_cond, output_uncond, infos, output, weights): 
    # FULL AND
    all_intervals =  infos["all_intervals"] # infos["original_timeline"] 
    lengths = infos["lengths"]
    ...
    for idx in range(real_nseq):
        ...
        for ii, x in enumerate(all_intervals[idx]):
            ...
            if hasattr(x, 'text'):
                # Spatial stiching
                val = weights[0] * (output_cond[ii_offset, start : end, :] - output_uncond[ii_offset, start : end, :])
...
def bada_bim_core(output_core, output_cond, output_uncond, infos, output, weights): #MYCODE
    # Versione di bada_bim in cui non si usa output_uncond ma core per fare anche le sottrazioni
    # FULL AND
    output_uncond = output_core  # DEBUG

    n_uncond = infos["n_uncond"][0] # non sono mica sicuro che sia giusto
    n_cond = output_cond.shape[0] - n_uncond
    all_intervals = infos["original_timeline"]# infos["all_intervals"]
    ...
    for idx in range(real_nseq):
        ...
        for ii, x in enumerate(all_intervals[idx]):
            ...
            if hasattr(x, 'text'):
                # Spatial stiching
                val = weights[0] * (output_cond[ii_offset, start : end, :] - output_core[idx, x.start : x.end, :])
                output[idx, x.start : x.end, :] += val
            # else:
                # Temporal stiching
                # val = output_uncond[ii, start : end, :]
