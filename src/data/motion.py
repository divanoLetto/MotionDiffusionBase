...
class AMASSMotionLoader:
    ...
    def __call__(self, path, start, end, drop_motion_perc=None, load_transition=False):
        ...
        if "humanact12" in path:  # MYCODE 
            length = int(end * self.fps) - int(start * self.fps)
            return {"x": torch.zeros(length, 205), "length": length}
        ...
        if load_transition:
            ...
            if drop_motion_perc is not None:
                ...
                # crop the motion
                if n_frames_right > 0 and n_frames_left > 0: # se sono entrambi zero si ottiene un vettore vuoto
