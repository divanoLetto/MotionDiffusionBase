...
def save_text_embeddings_mtt(path_, modelname, device="cuda"):
    model = TextToEmb(modelname, device=device)
    # annotations = load_annotations(path)

    path = os.path.join(path_, TextEmbeddings_mtt.name)
    ...
    path_MTT = f"{path_}/MTT"
    for filename in os.listdir(path_MTT):
        file = os.path.join(path_MTT, filename)
    ...
    # batch of N/10
    nb_tokens = []
    all_texts_batched = np.array_split(all_texts, 20)
